...
let prefix = await require('quick.db').fetch(`prefix_${message.guild.id}`) || ayarlar.prefix
  ...
  if (args[0] == 'aç') {
    db.set(`reklam_${message.channel.id}`, 'acik')
      ...
      .setColor("GREEN")
      .setTimestamp()
      .setDescription(`Bu Kanalda Başarıyla Maurx Reklam Engelleme Sistemi Açıldı!\nKapatmak için \`${prefix}reklam kapat\` Yazın!`)
      .setFooter(`${message.guild.name}`)
      message.channel.send(embed)
    message.react('703621502574002326')
  };
  if (args[0] == 'kapat') {
    ...
    .setColor('RED')
    .setTimestamp()
    .setDescription(`Bu Kanalda Başarıyla Maurx Reklam Engelleme Sistemi Kapatıldı!\nAçmak için \`${prefix}reklam aç\` Yazın!`)
    .setFooter(`${message.guild.name}`)
...
exports.help = {
  ...
  usage: 'm!reklam aç veya m!reklam kapat'
};