...
exports.run = async (client, message, args) => {
  ...
  if (!args[0])
    return message.channel.send("Lütfen aranacak emojinin adını giriniz.");
  const emojii = client.emojis.find(e => e.name == args[0]);

  if (emojii === null)
    return message.channel.send(
      "Böyle bir emoji bulunamadı. Unutmayın emojiler sadece botun **ekli** olduğu sunucularda aramaktadır."
    );
