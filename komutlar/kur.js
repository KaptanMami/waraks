...
module.exports.run = async (client, message, args) => {
  if (!message.member.hasPermission("ADMINISTRATOR"))
    return message.channel.send(
      `**Ne yazık ki bu komutu kullanabilmen için; "Yönetici" yetkisine sahip olman lazım!** <a:mavisiren:702294866859655318>`
    );
  try {
    ...
    if (!sa) {
      message.channel
        .send(
          "**Panel kuruluyor! Lütfen bekleyiniz!** \n**Not: Bu biraz zaman alabilir!** <a:gen:715570657777287278>"
        )
        .then(k => {
          setTimeout(() => {
            message.guild
              .createChannel("━━━Servers Panel━━━", "category", [
                { id: message.guild.id }
              ])
              .then(x => {
                setTimeout(() => {
      ...
      message.channel.send(
        "**Panel zaten kurulu!**\n**Sıfırlamak için: w!panel-sıfırla** "
      );
    }
  } catch (err) {
    const embed = new Discord.RichEmbed()
      .setColor("RED")
      .setDescription(
        `**Sanırım bir sorun var! Lütfen bunu destek sunucumuza gelip bildir!** [Destek Sunucumuz](https://discord.gg/waraks)`
      );
